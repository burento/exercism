...
Knapsack = Struct.new :max_weight do
  def max_value(items)
    values = (1..items.size).flat_map do |take_size|
      items.combination(take_size).filter_map do |combi|
        combi.sum(&:value) if combi.sum(&:weight) <= max_weight
      end
    end
    values.max || 0
  end
